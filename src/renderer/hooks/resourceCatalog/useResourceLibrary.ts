...
export function useResourceLibrary({
  ...
  const allResources = useMemo<ResourceItem[]>(
    () => (isAssistant ? baseAssistants.data.map(buildAssistantItem) : prompts.data.map(buildPromptItem)),
  ...
  const resources = useMemo<ResourceItem[]>(
    () => [...(isAssistant ? filteredAssistantItems : promptItems)].sort((a, b) => compareItems(a, b, sort)),
  ...
  }, [isAssistant, baseAssistantsRefetch, filteredAssistantsRefetch, groupsRefetch, promptsRefetch])
