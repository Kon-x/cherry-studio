...
describe('ModelSpeedControl UI', () => {
  ...
  it('uses a slider for GPT-5.6, with Off first and Default as a separate choice', () => {
    ...
    expect(screen.getByRole('button', { name: 'chat.speed.title' })).toHaveTextContent(
  ...
  it("displays a stored Default at the model's declared default without changing its submitted value", () => {
    ...
    expect(screen.getByRole('slider', { name: 'chat.speed.effort' })).toHaveAttribute(
      'aria-valuetext',
      'assistants.settings.reasoning_effort.default'
    )
  })
  ...
  it('changes one effort level per wheel step without scrolling the page', () => {
    ...
    const slider = screen.getByRole('slider', { name: 'chat.speed.effort' })
  ...
  it('resets an incomplete wheel step when the wheel target ref is rebound', () => {
    ...
    let slider = screen.getByRole('slider', { name: 'chat.speed.effort' })
    fireEvent.wheel(slider, { deltaY: -20 })

    rerender(<ControlledSpeedControl model={codexModel} initialEffort="high" />)
    slider = screen.getByRole('slider', { name: 'chat.speed.effort' })
    fireEvent.wheel(slider, { deltaY: -20 })
    expect(slider).toHaveAttribute('data-value', '3')
  ...
  it('toggles Fast only for a capable provider-model pair', () => {
    ...
    const fastButton = screen.getByRole('button', { name: 'chat.speed.fast' })
    expect(fastButton).toHaveAttribute('aria-pressed', 'false')
    fireEvent.click(fastButton)
    expect(fastButton).toHaveAttribute('aria-pressed', 'true')
    expect(screen.getByRole('button', { name: 'chat.speed.title' })).toHaveTextContent('chat.speed.fast')

    rerender(<ControlledSpeedControl model={{ ...codexModel, supportsFastMode: false }} initialEffort="max" />)
    expect(screen.queryByRole('button', { name: 'chat.speed.fast' })).not.toBeInTheDocument()
  })
  ...
  it('renders Fast without requiring reasoning options', () => {
    ...
    expect(screen.getByRole('button', { name: 'chat.speed.title' })).toHaveTextContent('chat.speed.label')
    expect(screen.getByRole('button', { name: 'chat.speed.fast' })).toBeInTheDocument()
    expect(screen.queryByTestId('reasoning-slider')).not.toBeInTheDocument()
  })
})
...
describe('ModelSpeedControl summary verbosity', () => {
  ...
  it('reports the picked verbosity and marks it selected', async () => {
    ...
    expect(screen.getByRole('button', { name: 'chat.speed.summary.concise' })).toHaveAttribute('aria-pressed', 'true')
    await userEvent.click(screen.getByRole('button', { name: 'chat.speed.summary.detailed' }))
  ...
  it('defaults to auto when nothing is stored', () => {
    ...
    expect(screen.getByRole('button', { name: 'chat.speed.summary.auto' })).toHaveAttribute('aria-pressed', 'true')
  })
  ...
  it('hides the row when the endpoint carries no summary knob', () => {
    ...
    expect(screen.queryByRole('button', { name: 'chat.speed.summary.auto' })).toBeNull()
  })
})
...
describe('ModelSpeedControl service tiers', () => {
  ...
  it('stays hidden when the model declares no speed control', () => {
    ...
    expect(screen.queryByRole('button', { name: 'chat.speed.title' })).not.toBeInTheDocument()
  })
  ...
  it('renders the four Groq tiers as an accessible vertical radio group and persists a selection', async () => {
    ...
    const group = screen.getByRole('radiogroup', { name: 'chat.speed.service_tier.label' })
    expect(group).toHaveAttribute('data-value', 'auto')
    expect(screen.getAllByRole('radio')).toHaveLength(4)
    await userEvent.click(screen.getByRole('radio', { name: 'chat.speed.service_tier.fast' }))
  ...
  it('renders only the three OpenRouter tiers', () => {
    ...
    expect(screen.queryByRole('radio', { name: 'chat.speed.service_tier.auto' })).not.toBeInTheDocument()
  })
  ...
  it('temporarily resolves an unsupported saved tier to Standard without changing the saved value', () => {
    ...
    expect(screen.getByRole('radiogroup', { name: 'chat.speed.service_tier.label' })).toHaveAttribute(
      'data-value',
      'standard'
    )
  })
