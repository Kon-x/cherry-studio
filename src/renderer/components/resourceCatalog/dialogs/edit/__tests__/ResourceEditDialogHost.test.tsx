...
describe('ResourceEditDialogHost', () => {
  ...
  it('keeps the target mounted until the shared unmount delay expires', async () => {
    ...
    render(<ResourceEditDialogHost target={{ kind: 'assistant', id: 'agent-1' }} onOpenChange={mocks.onOpenChange} />)
  ...
  it('reopens the same logical target and cancels its pending close', async () => {
    ...
    const { rerender } = render(
      <ResourceEditDialogHost target={{ kind: 'assistant', id: 'agent-1' }} onOpenChange={mocks.onOpenChange} />
    )
    ...
    await act(() => vi.advanceTimersByTime(DIALOG_UNMOUNT_DELAY_MS - 1))
    rerender(<ResourceEditDialogHost target={{ kind: 'assistant', id: 'agent-1' }} onOpenChange={mocks.onOpenChange} />)
