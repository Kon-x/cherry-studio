...
vi.mock('@data/services/ProviderService', () => ({
  providerService: { getByProviderId: mocks.providerGetByProviderId, getApiKeys: mocks.providerGetApiKeys }
}))
...
/** Loopback by default so the NO_PROXY exemption assertions below stay meaningful. */
const openAiProvider = (baseUrl = 'http://127.0.0.1:23333') => ({
  id: 'openai',
  authOptional: false,
...
describe('PdfTranslationService', () => {
  beforeEach(() => {
    ...
    mocks.providerGetApiKeys.mockReturnValue([{ key: 'sk-provider' }])
  ...
  it('uses the manually installed BabelDOC, routes the model at the provider directly, and returns the translated PDF', async () => {
  ...
  it('runs concurrent jobs independently, resolving each as its own sidecar finishes', async () => {
    type TestChild = EventEmitter & { stderr: PassThrough; stdout: PassThrough; kill: ReturnType<typeof vi.fn> }
    ...
    await expect(first).resolves.toMatchObject({ fileName: expect.stringContaining('research paper') })

    children.get('job-second')!.stdout.write(finishEvent())
    children.get('job-second')!.emit('close', 0, null)
    await expect(second).resolves.toMatchObject({ fileName: expect.stringContaining('research paper') })
  })
  ...
  describe('sidecar proxy environment', () => {
    ...
    it("inherits Cherry's proxy decision and exempts a loopback provider from it", async () => {
    ...
    it('exempts a loopback provider even when no proxy is configured', async () => {
      // Windows Python resolves proxies from the WinINET registry when the env carries none,
      // which sends the loopback call through the system proxy and 502s every paragraph.
      const env = await spawnedEnv()
    ...
    it('strips the brackets an IPv6 provider host carries', async () => {
      mocks.providerGetByProviderId.mockReturnValue(openAiProvider('http://[::1]:23333'))
    ...
    it('leaves a bypass list that already covers the provider untouched', async () => {
    ...
    it('keeps a remote provider on the configured proxy instead of exempting it', async () => {
      mocks.providerGetByProviderId.mockReturnValue(openAiProvider('https://api.openai.com'))
      vi.stubEnv('HTTPS_PROXY', 'http://127.0.0.1:7890')
