...
// BabelDOC's key is injected via babeldoc.toml for the selected provider, so
...
/**
 * BabelDOC only speaks the OpenAI chat-completions wire format. With the fork's API gateway gone
 * there is no bridge, so the provider must expose that endpoint itself.
 */
const resolveOpenAiCompatibleEndpoint = (providerId: string): { baseUrl: string; apiKey: string } => {
  const provider = providerService.getByProviderId(providerId)
  const rawBaseUrl = provider.endpointConfigs?.[ENDPOINT_TYPE.OPENAI_CHAT_COMPLETIONS]?.baseUrl
  if (!rawBaseUrl) {
    throw new Error(`Provider ${providerId} has no OpenAI-compatible endpoint for PDF translation`)
  }
  const apiKey = providerService.getApiKeys(providerId, { enabled: true })[0]?.key
  if (!apiKey && !provider.authOptional) throw new Error(`Provider ${providerId} has no enabled API key`)
  return { baseUrl: formatApiHost(rawBaseUrl), apiKey: apiKey ?? 'no-key-required' }
}
...
const isLoopbackHost = (host: string): boolean =>
  ['localhost', '127.0.0.1', '::1'].includes(host.replace(/^\[|\]$/g, '').toLowerCase())

/**
 * Proxy env for the sidecar.
 *
 * Two separate gaps break a loopback provider on Windows. Cherry's proxy decision lives in
 * `process.env` (written by ProxyService), not in the login shell the rest of the sidecar env is
 * filtered from — so the sidecar inherits nothing, and Python falls back to
 * `urllib.request.getproxies()`, which reads the WinINET registry and routes even the loopback call
 * through the system proxy; the proxy refuses it and every paragraph fails with 502. Pinning the
 * host into NO_PROXY is separately required: the registry's own `ProxyOverride` uses shapes like
 * `127.*` that httpx does not read as a bypass. Setting NO_PROXY alone is not enough either — it
 * makes `getproxies()` non-empty, which suppresses the registry fallback and sends BabelDOC's asset
 * downloads direct. Remote provider hosts must keep using the configured proxy, so they are not pinned.
 */
const buildSidecarProxyEnv = (inheritedNoProxy: string | undefined, providerHost: string): Record<string, string> => {
  ...
  const host = providerHost.replace(/^\[|\]$/g, '')
  if (isLoopbackHost(host) && !hosts.includes(host)) hosts.push(host)
...
export class PdfTranslationService extends BaseService {
  ...
  private async runTranslation(
    ...
    try {
      ...
      // removal) always pairs with it — a throw from the resolves above can't leave
      ...
      // Fork: the API gateway is removed, so the sidecar talks to the provider's own
      // OpenAI-compatible endpoint. Providers without one cannot drive BabelDOC.
      const { baseUrl, apiKey } = resolveOpenAiCompatibleEndpoint(providerId)
      const sidecarModelId = model.apiModelId ?? modelId
      this.throwIfCancelled(job)
      ...
      await this.runSidecar(job, executable, request, outputDir, sidecarModelId, baseUrl, apiKey, onProgress)
      this.throwIfCancelled(job)
  ...
  private async runSidecar(
    ...
    try {
      await new Promise<void>((resolve, reject) => {
        ...
        // endpoint resolution → mkdir → writeFile) killed a still-null child; re-check now so the
  ...
  private async buildSidecarEnv(providerBaseUrl: string): Promise<Record<string, string>> {
    ...
    return mergeBinaryExecutionEnv({
      ...allowedEnv,
      ...buildSidecarProxyEnv(allowedEnv.NO_PROXY ?? allowedEnv.no_proxy, new URL(providerBaseUrl).hostname),
      HOME: runtimeHome,
