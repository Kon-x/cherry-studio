...
export class AppUpdaterService extends BaseService {
  ...
  private async getUpdateRequest() {
    ...
    return { currentVersion, edition, ipCountry, region, updateChannel, updateHeaders }
  }

  private async configureUpdaterForCheck() {
    const { currentVersion, edition, ipCountry, region, updateChannel, updateHeaders } = await this.getUpdateRequest()
    ...
    logger.info(
      `Using fork update feed for version ${currentVersion}, edition: ${edition}, channel: ${updateChannel}, region: ${region} (IP country: ${ipCountry})`
    )
