...
**Same stem, different number — decide by role, not by the word.** `src/renderer/windows/` holds many renderer windows and is plural. `src/main/core/window/` is the singular namespace for window management. Apply the decision rule to the directory's contents instead of assuming that a word is always singular or plural.
